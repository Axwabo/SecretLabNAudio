...
public static partial class AudioPlayerExtensions
{
    ...
    extension(AudioPlayer player)
    {
        ...
        public AudioPlayer EnqueueShortClip(ClipName name, bool loop = false, float volume = 1)
            => player.EnqueueShortClip(name, ModifyChain.AmplifyIfNot1(volume), loop);
        ...
        /// <seealso cref="UseQueue(AudioPlayer,Action{AudioQueue})"/>
        /// <seealso cref="AudioQueueExtensions.EnqueueShortClip"/>
        public AudioPlayer EnqueueShortClip(ClipName name, ModifyChain? process, bool loop = false)
            => player.UseQueue(queue => queue.EnqueueShortClip(name, loop, process));
